...
class DirectionObservation(Observation):
    ...
    def compute_l0(self, network: 'Network') -> float:
        ...
        dx = tgt.x - sta.x
        dy = tgt.y - sta.y
        bearing = np.arctan2(dy, dx)  # Uhrzeigersinn: Nord=0, Ost=100 gon
        o = network.orientations.get(self.orientation_group, 0.0)
        l0 = bearing - o
        two_pi = 2 * np.pi
        while l0 - self.value > np.pi:  l0 -= two_pi
        while l0 - self.value < -np.pi: l0 += two_pi
        return l0
    ...
    def compute_A_row(self, network: 'Network', param_index: dict[str, int]) -> np.ndarray:
        ...
        # orientation bleibt -1.0 (korrekt)
